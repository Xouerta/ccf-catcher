...
def save_to_json(pcap_file, output_file):
    decoder = PcapDecode()
    packets = rdpcap(pcap_file)
    decoded_packets = []
    for packet in packets:
        decoded_data = decoder.ether_decode(packet)
        decoded_packets.append(decoded_data)
    with open(output_file, 'w', encoding='utf-8') as jsonfile:
        json.dump(decoded_packets, jsonfile, ensure_ascii=False, indent=4)

# 使用示例
pcap_file = 'test.pcap'
output_file = 'output.json'
save_to_json(pcap_file, output_file)